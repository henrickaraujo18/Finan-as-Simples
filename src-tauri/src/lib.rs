...
fn validate_email(value: &str) -> Result<String, String> {
    ...
    if email.len() < 5 || email.len() > 254 || !email.contains('@') || email.starts_with('@') || email.ends_with('@') {
...
fn sanitize_permissions(input: &Value) -> Value {
    ...
    for module in ["dashboard", "transactions", "accounts", "analytics", "openFinance", "investments", "exports", "settings", "users"] {
...
fn permission_allowed(permissions: &Value, module: &str, action: &str) -> bool {
    permissions
        .get(module)
        .and_then(|value| value.get(action))
        .and_then(Value::as_bool)
...
fn entity_module(entity_type: &str) -> &'static str {
    match entity_type {
...
fn configure_database(connection: &Connection) -> Result<(), String> {
    ...
    if !table_has_column(connection, "entities", "workspace_id")? {
        connection
            .execute("ALTER TABLE entities ADD COLUMN workspace_id TEXT NOT NULL DEFAULT ''", [])
...
fn audit(connection: &Connection, user_id: Option<&str>, workspace_id: Option<&str>, action: &str, details: Value) {
...
fn workspace_permissions(connection: &Connection, user_id: &str, workspace_id: &str) -> Result<Value, String> {
...
fn require_context(db: &LocalDb, auth: &AuthState) -> Result<(String, String, Value, bool), String> {
    let session = auth
        ...
        .clone();
    let user_id = session.user_id.ok_or_else(|| "autenticação necessária".to_string())?;
    ...
    let is_super_admin: bool = connection
        .query_row(
...
fn require_entity_action(entity_type: &str, permissions: &Value, action: &str) -> Result<(), String> {
...
fn upsert_in_transaction(
    ...
    let version = existing.as_ref().map(|(value, _, _)| value + 1).unwrap_or(1);
    ...
    transaction
        .execute(
            "INSERT INTO entities
             (id, workspace_id, entity_type, data_json, version, sync_state, deleted, created_at, updated_at)
            ...
            params![id, workspace_id, input.entity_type, payload, version, created_at, now],
        )
...
fn auth_status_internal(connection: &Connection, session: &AuthSession) -> Result<AuthStatus, String> {
    ...
    let workspaces = statement
        .query_map(params![user.id], |row| {
            ...
            Ok(WorkspaceSummary {
                ...
                name: row.get(1)?,
                role: row.get(2)?,
                permissions: serde_json::from_str(&permissions_raw).unwrap_or_else(|_| empty_permissions()),
            })
        })
        .map_err(|error| error.to_string())?
...
fn auth_setup_owner(
    ...
    let existing: i64 = connection
        ...
        .map_err(|error| error.to_string())?;
    if existing > 0 {
        return Err("a conta proprietária já foi configurada".to_string());
    }
    ...
    transaction
        .execute(
            "UPDATE entities SET workspace_id = ?1 WHERE workspace_id = ''",
            params![workspace_id],
        )
        .map_err(|error| format!("falha ao vincular dados existentes ao ambiente principal: {error}"))?;
    transaction.commit().map_err(|error| error.to_string())?;

    audit(&connection, Some(&user_id), Some(&workspace_id), "owner_setup", json!({"email": email}));
    drop(connection);
...
fn auth_login(
    ...
    let workspace_id: Option<String> = connection
        ...
        .optional()
        .map_err(|error| error.to_string())?;
    audit(&connection, Some(&user_id), workspace_id.as_deref(), "login", json!({}));
    drop(connection);
...
fn auth_switch_workspace(
    ...
    let session_snapshot = auth.session.lock().map_err(|_| "sessão indisponível".to_string())?.clone();
    let user_id = session_snapshot.user_id.ok_or_else(|| "autenticação necessária".to_string())?;
    let connection = db.connection.lock().map_err(|_| "banco local indisponível".to_string())?;
    let _: Value = workspace_permissions(&connection, &user_id, &workspace_id)?;
    audit(&connection, Some(&user_id), Some(&workspace_id), "workspace_switch", json!({}));
    drop(connection);
    {
        let mut session = auth.session.lock().map_err(|_| "sessão indisponível".to_string())?;
        session.active_workspace_id = Some(workspace_id);
...
fn auth_create_workspace(
    ...
    let (user_id, _, _, is_super_admin) = require_context(&db, &auth)?;
    if !is_super_admin {
        return Err("somente o administrador da plataforma pode criar ambientes".to_string());
    }
...
fn auth_list_members(
    ...
    let owner_id: String = connection
        .query_row("SELECT owner_user_id FROM workspaces WHERE id = ?1", params![workspace_id], |row| row.get(0))
        .map_err(|error| error.to_string())?;
    let mut statement = connection
        .prepare(
...
fn auth_create_or_grant_user(
    ...
    let existing_user: Option<String> = connection
    ...
    let user_id = if let Some(user_id) = existing_user {
        ...
        let password = input
            .temp_password
            .as_deref()
            .ok_or_else(|| "para um novo usuário, informe uma senha inicial".to_string())?;
        let password_hash = hash_password(password)?;
        let user_id = Uuid::new_v4().to_string();
        connection
            .execute(
    ...
    audit(
        ...
        "member_grant",
        json!({"targetUserId": user_id, "email": email, "role": role}),
    );
    drop(connection);
...
fn auth_update_member_permissions(
    ...
    let owner_id: String = connection
        .query_row("SELECT owner_user_id FROM workspaces WHERE id = ?1", params![input.workspace_id], |row| row.get(0))
        .map_err(|error| error.to_string())?;
    if input.user_id == owner_id {
        return Err("as permissões do proprietário não podem ser reduzidas".to_string());
    }
    connection
        .execute(
            "UPDATE memberships SET role = ?1, permissions_json = ?2, updated_at = ?3
             WHERE user_id = ?4 AND workspace_id = ?5 AND active = 1",
    ...
    audit(
        ...
        json!({"targetUserId": input.user_id, "role": role}),
    );
    drop(connection);
    auth_list_members(db, auth, input.workspace_id)
}
...
fn auth_remove_member(
    ...
    if caller_id == user_id {
        return Err("não é possível remover o próprio acesso por esta tela".to_string());
    }
    let connection = db.connection.lock().map_err(|_| "banco local indisponível".to_string())?;
    ...
    let owner_id: String = connection
        ...
        .map_err(|error| error.to_string())?;
    if user_id == owner_id {
        return Err("o proprietário do ambiente não pode ser removido".to_string());
    }
    connection
        .execute(
...
fn auth_change_password(
    ...
    let old_hash: String = connection
        ...
        .map_err(|error| error.to_string())?;
    if !verify_password(&current_password, &old_hash) {
...
fn auth_request_password_reset(email: String) -> Result<PasswordResetResponse, String> {
    ...
    Ok(PasswordResetResponse {
        accepted: true,
        cloud_provider_required: true,
        message: "Se existir uma conta vinculada a este e-mail, as instruções de recuperação serão enviadas quando o serviço de identidade em nuvem estiver ativado.".to_string(),
    })
}
...
fn list_entities(
    ...
    if !can_read_entity(&entity_type, &permissions) {
        return Ok(Vec::new());
    }
    let connection = db.connection.lock().map_err(|_| "banco local indisponível".to_string())?;
    let mut statement = connection
        ...
        .map_err(|error| error.to_string())?;
    let rows = statement
        .query_map(params![workspace_id, entity_type], entity_from_row)
        .map_err(|error| error.to_string())?;
    rows.collect::<Result<Vec<_>, _>>().map_err(|error| error.to_string())
}
...
fn upsert_entity(
    ...
    connection
        .query_row(
            "SELECT id, entity_type, data_json, version, sync_state, created_at, updated_at
             FROM entities WHERE id = ?1 AND workspace_id = ?2",
...
fn upsert_entities(
    ...
    for input in inputs {
        ids.push(upsert_in_transaction(&transaction, input, &workspace_id)?);
    }
    transaction.commit().map_err(|error| error.to_string())?;
...
fn delete_entity(
    ...
    let current_version: i64 = transaction
        .query_row(
            ...
            params![id, workspace_id, entity_type],
            |row| row.get(0),
        )
        .optional()
        .map_err(|error| error.to_string())?
        .ok_or_else(|| "registro não encontrado neste ambiente".to_string())?;

    transaction
        .execute(
            "UPDATE entities
             SET deleted = 1, version = ?1, sync_state = 'local', updated_at = ?2
             WHERE id = ?3 AND workspace_id = ?4 AND entity_type = ?5",
            params![current_version + 1, now, id, workspace_id, entity_type],
        )
        .map_err(|error| format!("falha ao excluir registro: {error}"))?;
    transaction.commit().map_err(|error| error.to_string())?;
...
fn export_snapshot(
    ...
    let (_, workspace_id, permissions, _) = require_context(&db, &auth)?;
    if !permission_allowed(&permissions, "exports", "view") {
        return Err("sem permissão para exportar dados".to_string());
    }
    let connection = db.connection.lock().map_err(|_| "banco local indisponível".to_string())?;
    let mut statement = connection
        .prepare(
        ...
        .map_err(|error| error.to_string())?;
    let rows = statement
        .query_map(params![workspace_id], entity_from_row)
        .map_err(|error| error.to_string())?
        .collect::<Result<Vec<_>, _>>()
        .map_err(|error| error.to_string())?;
    Ok(json!({
...
fn create_backup(
    ...
    let (_, _, _, is_super_admin) = require_context(&db, &auth)?;
    if !is_super_admin {
        return Err("somente o administrador da plataforma pode gerar o backup completo deste computador".to_string());
    }
...
async fn check_and_install_update(
    ...
    let Some(pubkey) = updater_public_key() else {
        return Ok(UpdateResult {
            ...
            available: false,
            version: None,
            message: "atualização automática aguardando a chave pública de assinatura".to_string(),
        });
    };
    ...
    let updater = app
        ...
        .map_err(|error| format!("falha ao configurar atualizador: {error}"))?
        .build()
    ...
    let Some(update) = updater
        .check()
        .await
        .map_err(|error| format!("falha ao verificar atualização: {error}"))?
    else {
        return Ok(UpdateResult {
            configured: true,
            available: false,
            version: None,
            message: "Finança Simples está atualizado".to_string(),
        });
    };
    ...
    update
        ...
        .await
        .map_err(|error| format!("falha ao instalar atualização {version}: {error}"))?;
...
pub fn run() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            auth_list_members,
            auth_create_or_grant_user,
            auth_update_member_permissions,
            auth_remove_member,
...
mod tests {
    ...
    #[test]
    fn banco_local_configura_schema_cinco() {
    ...
    #[test]
    fn permissoes_completas_incluem_open_finance_e_investimentos() {
        let permissions = full_permissions();
    ...
    fn upsert_isola_registro_por_workspace() {
        ...
        let id = upsert_in_transaction(
            &transaction,
            EntityInput {
                entity_type: "cards".to_string(),
                id: None,
                data: json!({"name":"Cartão teste","limitCents":100000}),
            },
            "workspace-1",
        )
        ...
        let workspace_id: String = connection
            .query_row("SELECT workspace_id FROM entities WHERE id = ?1", params![id], |row| row.get(0))
            .expect("ler workspace");
