...
fn request_headers(request: reqwest::RequestBuilder, token: &str) -> reqwest::RequestBuilder {
    request
...
async fn validate_user(http: &Client, token: &str) -> Result<CloudUser, String> {
    let response = request_headers(http.get(format!("{SUPABASE_URL}/auth/v1/user")), token)
        .send()
        .await
...
async fn fetch_platform_admin(http: &Client, token: &str, user_id: &str) -> Result<bool, String> {
    let response = request_headers(
        http.get(format!("{SUPABASE_URL}/rest/v1/profiles"))
            .query(&[("id", format!("eq.{user_id}")), ("select", "is_platform_admin".to_string())]),
        token,
    )
    .send()
    .await
...
fn placeholder_email(cloud_user_id: &str) -> String {
    let compact: String = cloud_user_id.chars().filter(|c| c.is_ascii_alphanumeric()).take(24).collect();
    format!("owner-{compact}@local.invalid")
}

fn reconcile_local_identity(
    db: &LocalDb,
    auth: &AuthState,
    ...
    let email = validate_email(cloud_user.email.as_deref().unwrap_or_default())?;
    ...
    let local_user_id = existing_local.unwrap_or_else(|| cloud_user.id.clone());
    ...
    transaction
        .execute(
            ...
            params![local_user_id, email, password_hash, if is_platform_admin { 1 } else { 0 }, now],
        )
        .map_err(|error| format!("falha ao reconciliar usuário local: {error}"))?;

    transaction
        .execute("UPDATE memberships SET active = 0, updated_at = ?1 WHERE user_id = ?2", params![now, local_user_id])
        .map_err(|error| error.to_string())?;

    for workspace in workspaces {
        let local_owner_id = if workspace.is_owner || workspace.owner_user_id == cloud_user.id {
            local_user_id.clone()
        } else {
            ...
            transaction
                .execute(
                    "INSERT OR IGNORE INTO users(id, email, password_hash, is_super_admin, active, created_at, updated_at)
                     VALUES (?1, ?2, '!', 0, 0, ?3, ?3)",
                    params![local_owner_id, placeholder_email(&workspace.owner_user_id), now],
                )
                .map_err(|error| error.to_string())?;
        }
            ...
            .execute(
                "INSERT INTO memberships(user_id, workspace_id, role, permissions_json, active, created_at, updated_at)
                 ...
                 ON CONFLICT(user_id, workspace_id) DO UPDATE SET
                   ...
                   active = 1,
                   updated_at = excluded.updated_at",
                params![local_user_id, workspace.id, workspace.role, workspace.permissions.to_string(), now],
            )
            .map_err(|error| format!("falha ao reconciliar permissões locais: {error}"))?;
    }

    // Migra dados do antigo workspace local para o primeiro workspace cloud somente quando
    // o workspace legado não faz parte da conta cloud. Isso preserva usuários vindos da 1.5.
    ...
    audit(&connection, Some(&local_user_id), workspaces.first().map(|w| w.id.as_str()), "cloud_identity_reconcile", json!({"cloudUserId": cloud_user.id}));
    drop(connection);

    {
        let mut session = auth.session.lock().map_err(|_| "sessão indisponível".to_string())?;
    ...
    auth_status(
        State::from_inner(db),
        State::from_inner(auth),
    )
}

// State::from_inner não existe em todas as versões do Tauri. Esta função replica o trecho final
// de auth_status sem depender de construir State manualmente.
...
pub(crate) async fn cloud_reconcile_login(
    ...
    let is_platform_admin = fetch_platform_admin(&http, &access_token, &user.id).await.unwrap_or(false);
    ...
    transaction.execute(
        "INSERT INTO users(id, email, password_hash, is_super_admin, active, created_at, updated_at)
         VALUES (?1, ?2, ?3, ?4, 1, ?5, ?5)
         ON CONFLICT(email) DO UPDATE SET password_hash=excluded.password_hash, is_super_admin=excluded.is_super_admin, active=1, updated_at=excluded.updated_at",
        params![local_user_id, email, password_hash, if is_platform_admin {1} else {0}, now],
    ).map_err(|error| error.to_string())?;
    transaction.execute("UPDATE memberships SET active=0, updated_at=?1 WHERE user_id=?2", params![now, local_user_id])
        .map_err(|error| error.to_string())?;

    for workspace in &workspaces {
        let local_owner_id = if workspace.is_owner || workspace.owner_user_id == user.id { local_user_id.clone() } else { workspace.owner_user_id.clone() };
        if local_owner_id != local_user_id {
            transaction.execute(
                "INSERT OR IGNORE INTO users(id,email,password_hash,is_super_admin,active,created_at,updated_at) VALUES (?1,?2,'!',0,0,?3,?3)",
                params![local_owner_id, placeholder_email(&workspace.owner_user_id), now],
            ).map_err(|error| error.to_string())?;
        }
        transaction.execute(
            "INSERT INTO workspaces(id,name,owner_user_id,active,created_at,updated_at) VALUES (?1,?2,?3,1,?4,?4)
             ON CONFLICT(id) DO UPDATE SET name=excluded.name, owner_user_id=excluded.owner_user_id, active=1, updated_at=excluded.updated_at",
            params![workspace.id, workspace.name, local_owner_id, now],
        ).map_err(|error| error.to_string())?;
        transaction.execute(
            "INSERT INTO memberships(user_id,workspace_id,role,permissions_json,active,created_at,updated_at) VALUES (?1,?2,?3,?4,1,?5,?5)
    ...
    transaction.commit().map_err(|error| error.to_string())?;
    audit(&connection, Some(&local_user_id), workspaces.first().map(|w| w.id.as_str()), "cloud_identity_reconcile", json!({"cloudUserId": user.id}));
    drop(connection);
...
pub(crate) async fn cloud_ensure_workspace(
    ...
    .json(&json!({"p_workspace_id": workspace_id, "p_name": name}))
    .send()
    .await
...
fn local_rows(connection: &Connection, workspace_id: &str) -> Result<Vec<LocalSyncEntity>, String> {
    let mut statement = connection.prepare(
        "SELECT id,entity_type,data_json,version,sync_state,deleted,created_at,updated_at FROM entities WHERE workspace_id=?1"
    ).map_err(|error| error.to_string())?;
    statement.query_map(params![workspace_id], |row| Ok(LocalSyncEntity {
        id: row.get(0)?,
...
async fn remote_rows(http: &Client, token: &str, workspace_id: &str) -> Result<Vec<RemoteEntity>, String> {
    let response = request_headers(
        http.get(format!("{SUPABASE_URL}/rest/v1/financial_entities")).query(&[
            ("workspace_id", format!("eq.{workspace_id}")),
            ("select", "id,workspace_id,entity_type,data_json,version,deleted,updated_by,created_at,updated_at".to_string()),
        ]), token)
        .send().await.map_err(|error| format!("falha ao baixar dados cloud: {error}"))?;
    if !response.status().is_success() {
        return Err(response_error(response, "falha ao baixar dados cloud").await);
    }
    response.json::<Vec<RemoteEntity>>().await.map_err(|error| format!("dados cloud inválidos: {error}"))
}
...
fn apply_remote(connection: &Connection, remote: &RemoteEntity) -> Result<(), String> {
    connection.execute(
        "INSERT INTO entities(id,workspace_id,entity_type,data_json,version,sync_state,deleted,created_at,updated_at)
         ...
         ON CONFLICT(id) DO UPDATE SET workspace_id=excluded.workspace_id, entity_type=excluded.entity_type,
           data_json=excluded.data_json, version=excluded.version, sync_state='synced', deleted=excluded.deleted,
           created_at=excluded.created_at, updated_at=excluded.updated_at",
        params![remote.id, remote.workspace_id, remote.entity_type, remote.data_json.to_string(), remote.version, if remote.deleted {1} else {0}, remote.created_at, remote.updated_at],
...
async fn upload_local(
    ...
    let request = if exists_remote {
        http.patch(format!("{SUPABASE_URL}/rest/v1/financial_entities"))
            .query(&[("workspace_id", format!("eq.{workspace_id}")), ("id", format!("eq.{}", local.id))])
    } else {
        http.post(format!("{SUPABASE_URL}/rest/v1/financial_entities"))
    };
    let response = request_headers(request, token)
        .header("Prefer", "return=minimal")
        .json(&payload)
        .send().await.map_err(|error| format!("falha ao enviar alteração cloud: {error}"))?;
    if !response.status().is_success() {
...
pub(crate) async fn cloud_sync(
    ...
    let workspaces = fetch_workspaces(&http, &access_token).await?;
    let session = auth.session.lock().map_err(|_| "sessão indisponível".to_string())?.clone();
    let workspace_id = session.active_workspace_id.ok_or_else(|| "nenhum ambiente selecionado".to_string())?;
    let cloud_workspace = workspaces.iter().find(|workspace| workspace.id == workspace_id)
        .ok_or_else(|| "acesso cloud a este ambiente foi revogado".to_string())?;

    let (locals, permissions) = {
        let connection = db.connection.lock().map_err(|_| "banco local indisponível".to_string())?;
        (local_rows(&connection, &workspace_id)?, cloud_workspace.permissions.clone())
    };
    let remotes = remote_rows(&http, &access_token, &workspace_id).await?;
    let remote_map: HashMap<String, RemoteEntity> = remotes.iter().cloned().map(|row| (row.id.clone(), row)).collect();
    let local_map: HashMap<String, LocalSyncEntity> = locals.iter().cloned().map(|row| (row.id.clone(), row)).collect();
    ...
    for local in &locals {
        ...
        let remote_newer = remote.map(|r| r.version > local.version || (r.version == local.version && r.updated_at > local.updated_at)).unwrap_or(false);
        ...
        if local_dirty && !remote_newer {
            ...
            if permission_allowed(&permissions, module, action) {
                upload_local(&http, &access_token, &cloud_user.id, &workspace_id, local, remote.is_some()).await?;
                uploaded += 1;
...

    {
        ...
        for remote in &remotes {
            ...
            let local_dirty = local.map(|l| l.sync_state != "synced").unwrap_or(false);
            let local_newer = local.map(|l| l.version > remote.version || (l.version == remote.version && l.updated_at > remote.updated_at)).unwrap_or(false);
            if local.is_none() || (!uploaded_ids.contains(&remote.id) && (!local_dirty || !local_newer)) {
        ...
        for local in &locals {
            if uploaded_ids.contains(&local.id) {
                connection.execute(
                    "UPDATE entities SET sync_state='synced' WHERE id=?1 AND workspace_id=?2 AND version=?3",
        ...
        audit(&connection, session.user_id.as_deref(), Some(&workspace_id), "cloud_sync", json!({"uploaded":uploaded,"downloaded":downloaded,"skipped":skipped,"pending":pending}));
        return Ok(CloudSyncResult {
            ...
            message: if pending == 0 { "Sincronização concluída".to_string() } else { format!("Sincronização parcial: {pending} alteração(ões) pendente(s)") },
        });
    }
}
