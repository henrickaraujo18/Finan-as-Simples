...
1. Criar projeto.
2. Aplicar `supabase/migrations/001_identity_workspaces.sql`.
...
4. Implantar `invite-user` com autenticação obrigatória.
...
7. Preencher no desktop somente `supabaseUrl`, chave publicável e URL de recuperação.
8. Executar Security Advisor e testes de isolamento entre dois usuários/workspaces antes de habilitar sincronização real em produção.
